...
模块中，仅需配置每个 pathname 和其根视图之间的对应关系。详见: @GUIDE[Maigx View模型]

根视图往往决定页面的基础布局，多数 app 的基础布局很少改变（如：头、尾、侧边、主区域模式），
所以多数情况下 pathname 对应着相同的视图，即 defaultView。

* 本例中，routes 方法定义了任意 pathname，都采用 defaultView
* defaultView 的值是一个视图的模块名，本例中是 `app/common/views/default`
