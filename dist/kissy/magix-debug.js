 ...
 */
    ...
    var G_ToTry = function(fns, context, args, i, r, e) {
    ...
    var Vframe_GetVframes = function(id, node, arr) {
        node = G_GetById(id);
        if (node) {
            arr = S.all('[mx-vframe]', node);
        }
        return arr || G_EMPTY_ARRAY;
    };
    ...
    var Vframe_NotifyAlter = function(vframe, e, mId, p) {
        if (!e) e = {};
        if (!vframe.$ca && vframe.$cr) { //当前vframe触发过created才可以触发alter事件
            vframe.$cr = 0;
            vframe.$ca = 1;
            vframe.fire('alter', e);
            mId = vframe.id;
    ...
    G_Mix(G_Mix(Vframe[G_PROTOTYPE], Event), {
        ...
        mountView: function(viewPath, viewInitParams) {
            ...
            if (!me.$a && node) {
                me.$a = 1;
                me.$t = G_HTML(node); //.replace(ScriptsReg, '');
            }
        ...
        unmountView: function() {
            ...
            if (view) {
                ...
                if (node && me.$a) { //如果view本身是没有模板的，也需要把节点恢复到之前的状态上：只有保留模板且view有模板的情况下，这条if才不执行，否则均需要恢复节点的html
                    G_HTML(node, me.$t);
                }
        ...
        mountVframe: function(id, viewPath, viewInitParams /*, cancelTriggerEvent*/ ) {
            ...
            if (me.$cr /*&& !cancelTriggerEvent*/ ) Vframe_NotifyAlter(me); //如果在就绪的vframe上渲染新的vframe，则通知有变化
            //var vom = me.owner;
            vf = Vframe_Vframes[id];
            if (!vf) {
                if (!G_Has(me.$c, id)) {
        ...
        mountZone: function(zoneId, viewInitParams) {
            ...
            var vframes = Vframe_GetVframes(zoneId);
            /*
                body(#mx-root)
         ...
         * @param  {Array} args 参数
         * @param {Boolean} override 是否覆盖未调用的同名方法
         * @return {Object}
         */
        invoke: function(name, args, override) {
            var result;
            var vf = this,
                view, fn, o, list, key;
            if ((view = vf.$v) && view.$p) {
                result = (fn = view[name]) && G_ToTry(fn, view, args);
            } else {
                list = vf.$il;
 ...
 */
    ...
    var Body_DOMEventProcessor = function(e) {
        ...
        while (current != G_DOCBODY) { //找事件附近有mx-[a-z]+事件的DOM节点
            ...
            if ((ignore = current.$) && ignore[eventType] /*|| e.isPropagationStopped()*/ ) {
                break;
            } else {
                arr.push(current);
            }
    ...
    var View_WrapRender = function(prop, fn, me) {
        ...
        prop.render = function() {
            ...
            if (me.$s > 0) {
                ...
                G_ToTry(fn, me, G_Slice.call(arguments));
            }
        };
    };
    /**
            ...
            for (p in prop) {
                ...
                if (temp) {
                    ...
                    while ((temp = evts.pop())) {
                        ...
                        } else if (!prop[node = name + G_SPLITER + temp]) {
                            eventsObj[temp] = 1;
                            prop[node] = old;
                        }
                    }
