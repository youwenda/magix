...
tmplData:[{"guid":1,"keys":["fields","checkedMap"],"tmpl":"<%for(var i=0;i<fields.length;i++){%><label><input type=\"checkbox\" <%if(checkedMap[fields[i].id]){%> checked=\"checked\" <%}%> mx-click=\"toggleField({id:'<%=fields[i].id%>'})\"/><%=fields[i].text%></label><%}%>","selector":"div[mx-guid=\"xa3f1-\u001f\"]"},{"guid":2,"keys":["checkedFields"],"tmpl":"<%for(var i=0;i<checkedFields.length;i++){%><li dragdrop=\"v\" id=\"sf_<%=checkedFields[i]%>_<%=id%>\" mx-mousedown=\"dragStart()\" index=\"<%=i%>\"><span><%=fieldsMap[checkedFields[i]].text%></span><i style=\"display: inline-block;float:right\">xx</i></li><%}%>","selector":"ul[mx-guid=\"xa3f2-\u001f\"]"},{"guid":3,"keys":["checkedFields"],"tmpl":"<ul style=\"width:<%=94*checkedFields.length%>px\"><%for(var i=0;i<checkedFields.length;i++){%><li dragdrop=\"h\" id=\"sf_<%=checkedFields[i]%>_<%=id%>\" mx-mousedown=\"dragStartH()\" index=\"<%=i%>\"><span><%=fieldsMap[checkedFields[i]].text%></span><i style=\"display: inline-block;float:right\">xx</i></li><%}%></ul>","selector":"div[mx-guid=\"xa3f3-\u001f\"]"}],
    ctor: function() {
        var me = this;
