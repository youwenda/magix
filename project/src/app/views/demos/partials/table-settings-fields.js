...
return Picker.extend({
    tmpl: "<div class=\"mp-fec-cnt\"><div class=\"mp-fec-fields-wrapper clearfix\"><div class=\"mp-fec-fields\" mx-guid=\"xcd91-\u001f\">@1-\u001f</div><div class=\"mp-fec-sortable\" id=\"sortable_<%=id%>\"><ul mx-guid=\"xcd92-\u001f\">@2-\u001f</ul></div></div><div class=\"mp-fec-h-fields\" mx-guid=\"xcd93-\u001f\" id=\"hsortable_<%=id%>\">@3-\u001f</div><div class=\"mp-fec-buttons\"><button class=\"btn\" mx-click=\"enter();\">确定</button><button class=\"btn\" mx-click=\"cancel();\">取消</button></div></div>",
tmplData:[{"guid":1,"keys":["fields","checkedMap"],"tmpl":"\n            <%for(var i=0;i<fields.length;i++){%>\n                <label><input type=\"checkbox\"<%if(checkedMap[fields[i].id]){%> checked=\"checked\"<%}%> mx-click=\"toggleField({id:'<%=fields[i].id%>'})\" /><%=fields[i].text%>\n                </label>\n            <%}%>\n        ","selector":"div[mx-guid=\"xcd91-\u001f\"]","attrs":[]},{"guid":2,"keys":["checkedFields"],"tmpl":"\n                <%for(var i=0;i<checkedFields.length;i++){%>\n                    <li dragdrop=\"v\" id=\"sf_<%=checkedFields[i]%>_<%=id%>\" mx-mousedown=\"dragStart()\" index=\"<%=i%>\"><span><%=fieldsMap[checkedFields[i]].text%></span><i style=\"display: inline-block;float:right\">xx</i></li>\n                <%}%>\n            ","selector":"ul[mx-guid=\"xcd92-\u001f\"]","attrs":[]},{"guid":3,"keys":["checkedFields"],"tmpl":"<ul style=\"width:<%=94*checkedFields.length%>px\">\n            <%for(var i=0;i<checkedFields.length;i++){%>\n                <li dragdrop=\"h\" id=\"sf_<%=checkedFields[i]%>_<%=id%>\" mx-mousedown=\"dragStartH()\" index=\"<%=i%>\"><span><%=fieldsMap[checkedFields[i]].text%></span><i style=\"display: inline-block;float:right\">xx</i></li>\n            <%}%>\n        </ul>","selector":"div[mx-guid=\"xcd93-\u001f\"]","attrs":[]}],
    ctor: function() {
        var me = this;
