...
return View.extend({
    tmpl: "<div class=\"mp-1cd-wrapper\"><ul mx-guid=\"x3de1-\u001f\">@1-\u001f</ul></div><hr/><div class=\"mp-1cd-wrapper\"><ul mx-guid=\"x3de2-\u001f\">@2-\u001f</ul></div><button mx-click=\"saveSnapshot()\" class=\"btn\">保存当前数据</button>",
tmplData:[{"guid":1,"keys":["list"],"tmpl":"<li><input class=\"input\" type=\"text\" value=\"<%=list[0].test%>\" mx-change=\"setValue({path:'list.0.test'})\" id=\"list_0\" mx-focusin=\"removeError()\"/></li><%\n            for(var i=1;i<list.length;i++){\n        %><li><input mx-focusin=\"removeError()\" class=\"input\" type=\"text\" mx-change=\"setValue({path:'list.<%=i%>.test'})\" value=\"<%=list[i].test%>\" id=\"list_<%=i%>\"/><button class=\"btn\" mx-click=\"remove({index:<%=i%>})\" style=\"margin-left:10px\">X</button></li><%\n            }\n        %><li><button mx-click=\"add()\" class=\"btn\">添加一个</button></li>","selector":"ul[mx-guid=\"x3de1-\u001f\"]"},{"guid":2,"keys":["platforms"],"tmpl":"<%\n            for(var i=0;i<platforms.length;i++){\n        %><li><div mx-view=\"app/views/demos/partials/dropdown?type=platform&selected=<%=platforms[i].platformId%>\" mx-change=\"setValue({path:'platforms.<%=i%>.platformId'})\" class=\"mp-1cd-fl mp-1cd-ml10\" id=\"p_<%=i%>\" mx-focusin=\"removeError()\"></div><div mx-view=\"app/views/demos/partials/dropdown?type=operator&selected=<%=platforms[i].operatorId%>\" mx-change=\"setValue({path:'platforms.<%=i%>.operatorId'})\" class=\"mp-1cd-fl mp-1cd-ml10\" id=\"o_<%=i%>\" mx-focusin=\"removeError()\"></div><input class=\"input mp-1cd-w80 mp-1cd-ml10\" type=\"text\" mx-change=\"setValue({path:'platforms.<%=i%>.version'})\" value=\"<%=platforms[i].version%>\" placeholder=\"版本号\" id=\"v_<%=i%>\" mx-focusin=\"removeError()\"/> <button class=\"btn mp-1cd-ml10\" mx-click=\"removePlatform({index:<%=i%>})\">X</button></li><%\n            }\n        %><li><button mx-click=\"addPlatform()\" class=\"btn\">添加一个平台</button> <button mx-click=\"clearPlatform()\" class=\"btn\">清除平台</button> <button mx-click=\"clearOnePlatform()\" class=\"btn\">清除一个平台</button></li>","selector":"ul[mx-guid=\"x3de2-\u001f\"]"}],
    ctor: function() {
        var me = this;
