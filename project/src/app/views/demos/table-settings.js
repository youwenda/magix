...
return Magix.View.extend({
    tmpl: "<div class=\"mp-197-wrapper\"><button class=\"btn\" mx-click=\"showSettings()\">设置</button><button class=\"btn mp-197-fr\" mx-click=\"showSettings({dock:'right'})\">设置</button><table class=\"table\"><thead><tr mx-guid=\"x0b61-\u001f\">@1-\u001f</tr></thead><tbody mx-guid=\"x0b62-\u001f\">@2-\u001f</tbody><tfoot mx-mousedown=\"drag()\"><tr><td colspan=\"4\">xxx</td></tr></tfoot></table></div>",
tmplData:[{"guid":1,"keys":["checkedFields"],"tmpl":"<th>名称</th>\n                <%for(var i=0;i<checkedFields.length;i++){%>\n                <th><%=fieldsMap[checkedFields[i]].text%></th>\n                <%}%>\n            ","selector":"tr[mx-guid=\"x0b61-\u001f\"]","attrs":[]},{"guid":2,"keys":["checkedFields"],"tmpl":"\n            <%for(var i=0;i<20;i++){%>\n            <tr><td>测试<%=Math.random()%></td>\n                <%for(var j=0;j<checkedFields.length;j++){%>\n                <td><%=fieldsMap[checkedFields[j]].text+Math.random()%></td>\n                <%}%>\n            </tr>\n            <%}%>\n        ","selector":"tbody[mx-guid=\"x0b62-\u001f\"]","attrs":[]}],
    ctor: function() {
        var me = this;
