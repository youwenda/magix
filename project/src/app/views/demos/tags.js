...
return Magix.View.extend({
    tmpl: "<div class=\"mp-997-m100\"><div class=\"mp-997-tags clearfix\" id=\"tags_<%=id%>\" mx-click=\"showList()\" mx-guid=\"xb161-\u001f\">@1-\u001f</div><div class=\"mp-997-list mp-997-none\" id=\"list_<%=id%>\"><ul mx-guid=\"xb162-\u001f\">@2-\u001f</ul></div></div><button mx-click=\"test()\"></button>",
tmplData:[{"guid":1,"keys":["selected"],"tmpl":"<%for(var i=0;i<selected.length;i++){%><div class=\"mp-997-item\" id=\"si_<%=i%>_<%=id%>\" title=\"<%=selected[i]%>\"><div class=\"mp-997-name ellipsis\"><%=selected[i]%></div><div class=\"mp-997-delete\" mx-click=\"remove({index:<%=i%>})\">x</div></div><%}%><div class=\"mp-997-item\" id=\"iptw_<%=id%>\"><input id=\"ipt_<%=id%>\" class=\"mp-997-ipt\" mx-keyup=\"search()\" mx-paste=\"search()\" value=\"<%=iptValue%>\"/></div>","selector":"div[mx-guid=\"xb161-\u001f\"]"},{"guid":2,"keys":["list"],"tmpl":"<%for(var i=0;i<list.length;i++){%><li id=\"idx_<%=i%>\" class=\"mp-997-li ellipsis\" mx-mouseout=\"hover()\" mx-mouseover=\"hover()\" mx-click=\"fill({text:'<%=list[i].text%>'})\" data-text=\"<%=list[i].text%>\"><%=list[i].text%></li><%}%>","selector":"ul[mx-guid=\"xb162-\u001f\"]"}],
    ctor: function() {
        Monitor.setup();
        var me = this;
