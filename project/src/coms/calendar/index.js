...
return Magix.View.extend({
    tmpl: "<div class=\"mp-ec1-wrapper\"><table id=\"days_<%=id%>\" class=\"mp-ec1-table\"><thead><tr><th mx-click=\"changeMonth()\" class=\"mp-ec1-turn mp-ec1-title\">⇦</th><th colspan=\"5\" mx-guid=\"xdfe1-\u001f\" mx-click=\"showMonths()\" class=\"mp-ec1-title\">@1-\u001f</th><th mx-click=\"changeMonth({next:true})\" class=\"mp-ec1-turn mp-ec1-title\">⇨</th></tr></thead><tbody mx-guid=\"xdfe2-\u001f\">@2-\u001f</tbody></table><table id=\"months_<%=id%>\" class=\"mp-ec1-table mp-ec1-none\"><thead><tr><th mx-click=\"changeYear()\" class=\"mp-ec1-turn mp-ec1-title\">⇦</th><th mx-guid=\"xdfe3-\u001f\" mx-click=\"showYears()\" class=\"mp-ec1-title\">@3-\u001f</th><th mx-click=\"changeYear({next:true})\" class=\"mp-ec1-turn mp-ec1-title\">⇨</th></tr></thead><tbody><tr mx-guid=\"xdfe4-\u001f\">@4-\u001f</tr></tbody></table><table id=\"years_<%=id%>\" class=\"mp-ec1-table mp-ec1-none\"><thead><tr><th mx-click=\"changeYear({range:true})\" class=\"mp-ec1-turn mp-ec1-title\">⇦</th><th mx-guid=\"xdfe5-\u001f\" class=\"mp-ec1-title\">@5-\u001f</th><th mx-click=\"changeYear({range:true,next:true})\" class=\"mp-ec1-turn mp-ec1-title\">⇨</th></tr></thead><tbody><tr><td colspan=\"3\" mx-guid=\"xdfe6-\u001f\">@6-\u001f</td></tr></tbody></table></div>",
tmplData:[{"guid":1,"keys":["year","month"],"tmpl":"<%=year%>-<%=month<10?'0'+month:month%>","selector":"th[mx-guid=\"xdfe1-\u001f\"]"},{"guid":2,"keys":["days","weekText"],"tmpl":"<tr><%for(var i=0;i<weekText.length;i++){%><td class=\"mp-ec1-week\"><%=weekText[i]%></td><%}%></tr><%for(var i=0;i<days.length;i++){%><tr><%var tds=days[i];for(var j=0;j<tds.length;j++){%><td class=\"mp-ec1-day<%if(tds[j].otherMonth){%> mp-ec1-gray<%}if(tds[j].disabled){%> mp-ec1-disabled<%}if(tds[j].selected){%> mp-ec1-selected<%}%>\" <%if(!tds[j].disabled){%> mx-click=\"choose({toMonth:<%=tds[j].month%>,date:'<%=tds[j].full%>'})\" <%}%> title=\"<%=tds[j].full%>\"><%=tds[j].day%></td><%}%></tr><%}%>","selector":"tbody[mx-guid=\"xdfe2-\u001f\"]"},{"guid":3,"keys":["year"],"tmpl":"<%=year%>","selector":"th[mx-guid=\"xdfe3-\u001f\"]"},{"guid":4,"keys":["months"],"tmpl":"<td colspan=\"3\"><%for(var i=0,m;i<months.length;i++){m=months[i]%><span class=\"mp-ec1-year<%if(m.disabled){%> mp-ec1-disabled<%}if(m.selected){%> mp-ec1-selected<%}%>\" <%if(!m.disabled){%> mx-click=\"pickMonth({month:'<%=m.month%>'})\" <%}%>><%=m.month%></span><%}%></td>","selector":"tr[mx-guid=\"xdfe4-\u001f\"]"},{"guid":5,"keys":["startYear","endYear"],"tmpl":"<%=startYear%>-<%=endYear%>","selector":"th[mx-guid=\"xdfe5-\u001f\"]"},{"guid":6,"keys":["years"],"tmpl":"<%for(var i=0,y;i<years.length;i++){y=years[i]%><span class=\"mp-ec1-year<%if(y.selected){%> mp-ec1-selected<%}if(y.disabled){%> mp-ec1-disabled<%}if(!i||i==years.length-1){%> mp-ec1-gray<%}%>\" <%if(!y.disabled){%> mx-click=\"pickYear({year:'<%=y.year%>'})\" <%}%>><%=y.year%></span><%}%>","selector":"td[mx-guid=\"xdfe6-\u001f\"]"}],
    update: function(ops) {
        ops = ops || {};
