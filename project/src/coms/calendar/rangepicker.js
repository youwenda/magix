...
var Rangepicker = Picker.extend({
    tmpl: "<%if(quickDates.length){%>\n    <div class=\"mp-bd5-shortcuts-header\">快捷日期</div><div class=\"mp-bd5-shortcuts mp-bd5-clearfix\" mx-guid=\"x8431-\u001f\">@1-\u001f</div>\n<%}%>\n<div class=\"mp-bd5-shortcuts-header\">日期范围</div><div class=\"mp-bd5-datepicker mp-bd5-clearfix\" mx-guid=\"x8432-\u001f\">@2-\u001f</div><div class=\"mp-bd5-buttons mp-bd5-clearfix\"><button mx-click=\"picked()\" class=\"btn\">确定</button><button mx-click=\"hide()\" class=\"btn\">取消</button></div>",
tmplData:[{"guid":1,"keys":["dates"],"tmpl":"\n        <%for(var i=0;i<quickDates.length;i++){%>\n            <%var key=quickDates[i],info=quickDatesMap[key]%>\n            <a<%if(dates.quickDateKey==key){%> class=\"mp-bd5-selected\"<%}%> mx-click=\"picked({quick:true,key:'<%=key%>'})\"><%=info?info.text:key%></a>\n        <%}%>\n    ","selector":"div[mx-guid=\"x8431-\u001f\"]","attrs":[]},{"guid":2,"keys":["dates"],"tmpl":"<input class=\"input\" value=\"<%=dates.startStr%>\" readonly=\"readonly\" mx-click=\"showDatepicker({first:true})\" id=\"start_<%=id%>\" />-<input class=\"input\" value=\"<%=dates.endStr%>\" readonly=\"readonly\" mx-click=\"showDatepicker()\" id=\"end_<%=id%>\" />","selector":"div[mx-guid=\"x8432-\u001f\"]","attrs":[]}],
    ctor: function(ops) {
        var me = this;
        me.$dates = ops.dates;
