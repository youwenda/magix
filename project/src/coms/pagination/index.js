...
return Magix.View.extend({
    tmpl: "<div class=\"mp-886-page-list\" mx-guid=\"xa201-\u001f\">@1-\u001f</div>",
tmplData:[{"guid":1,"keys":["start","end","index","path","pages"],"tmpl":"<a href=\"#!<%=path%>\"<%if(index==1){%> class=\"mp-886-page-disabled\"<%}else{%> mx-click=\"toPrev()\"<%}%>>⇦</a>\n    <%if(start>1){%>\n    <a href=\"#!<%=path%>\" mx-click=\"toPage({page:1})\">1</a>\n    <%}%>\n    <%if(start>2){%>\n        <a class=\"page-others\">...</a>\n    <%}%>\n    <%for(var i=start;i<=end;i++){%>\n        <a class=\"page-item<%if(i==index){%> mp-886-page-active<%}%>\" href=\"#!<%=path%>\" mx-click=\"toPage({page:<%=i%>})\"><%=i%></a>\n    <%}%>\n    <%if(end+2<=pages){%>\n        <a class=\"page-others\">...</a>\n    <%}%>\n    <%if(end<pages){%>\n        <a href=\"#!<%=path%>\" mx-click=\"toPage({page:<%=pages%>})\"><%=pages%></a>\n    <%}%>\n    <a href=\"#!<%=path%>\"<%if(index==pages){%> class=\"mp-886-page-disabled\"<%}else{%> mx-click=\"toNext()\"<%}%>>⇨</a>","selector":"div[mx-guid=\"xa201-\u001f\"]","attrs":[]}],
    render: function() {
        var me = this;
        me.endUpdate();
