...
tmplData:[{"guid":1,"keys":["titleText","width"],"tmpl":"<span class=\"ellipsis mp-740-ib\" style=\"width:<%=width-15%>px\" id=\"title_<%=id%>\" title=\"<%=titleText%>\">\n            <%=titleText%>\n        </span><span class=\"mp-740-icon\" id=\"icon_<%=id%>\">⇩</span>","selector":"div[mx-guid=\"xf961-\u001f\"]","attrs":[{"n":"style","v":"width:<%=width%>px"}]},{"guid":2,"keys":["list","selected","width","height"],"tmpl":"\n        <%for(var i=0,one;i<list.length;i++){%>\n        <%one=list[i]%>\n        <li mx-mouseover=\"hover()\" mx-mouseout=\"hover();\" mx-click=\"select({id:'<%=one.id%>'})\" class=\"mp-740-li ellipsis<%if(selected==one.id){%> mp-740-selected<%}%>\" title=\"<%=one.text%>\">\n        <%=one.text%>\n        </li>\n        <%}%>\n    ","selector":"ul[mx-guid=\"xf963-\u001f\"]","attrs":[{"n":"style","v":"width:<%=width+9%>px;<%if(height){%>max-height:<%=height%>px;overflow:auto<%}%>"}]},{"keys":["width"],"selector":"input[mx-guid=\"xf962-\u001f\"]","attrs":[{"n":"style","v":"width:<%=width-10%>px"}]}],
    ctor: function(extra) {
        var me = this;
        Monitor.setup();
