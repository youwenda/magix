...
var CSSNames = {"none":"mp-740-none","over":"mp-740-over","header-active":"mp-740-header-active"};
var Monitor = require('../bases/monitor');
return Magix.View.extend({
    tmpl: "<div mx-click=\"toggle();\"\n     class=\"mp-740-header\"\n     id=\"header_<%=id%>\"\n     style=\"width:<%=width%>px\" mx-guid=\"xf961-\u001f\">@1-\u001f</div><div id=\"list_<%=id%>\" class=\"mp-740-items mp-740-none\">\n    <%if(search){%>\n    <input class=\"input mp-740-ipt\" mx-guid=\"xf962-\u001f\" mx-keyup=\"search()\" mx-paste=\"search()\" style=\"width:<%=width-10%>px\" />\n    <%}%>\n    <ul class=\"mp-740-list\" mx-guid=\"xf963-\u001f\" style=\"width:<%=width+9%>px;<%if(height){%>max-height:<%=height%>px;overflow:auto<%}%>\">@2-\u001f</ul></div>",
tmplData:[{"guid":1,"keys":["titleText","width"],"tmpl":"<span class=\"ellipsis mp-740-ib\" style=\"width:<%=width-15%>px\" id=\"title_<%=id%>\" title=\"<%=titleText%>\">\n            <%=titleText%>\n        </span><span class=\"mp-740-icon\" id=\"icon_<%=id%>\">⇩</span>","selector":"div[mx-guid=\"xf961-\u001f\"]","attrs":[{"n":"style","v":"width:<%=width%>px"}],"mask":"1011"},{"guid":2,"keys":["list","selected","width","height"],"tmpl":"\n        <%for(var i=0,one;i<list.length;i++){%>\n        <%one=list[i]%>\n        <li mx-mouseover=\"hover()\" mx-mouseout=\"hover();\" mx-click=\"select({id:'<%=one.id%>'})\" class=\"mp-740-li ellipsis<%if(selected==one.id){%> mp-740-selected<%}%>\" title=\"<%=one.text%>\">\n        <%=one.text%>\n        </li>\n        <%}%>\n    ","selector":"ul[mx-guid=\"xf963-\u001f\"]","attrs":[{"n":"style","v":"width:<%=width+9%>px;<%if(height){%>max-height:<%=height%>px;overflow:auto<%}%>"}],"mask":"10100101"},{"keys":["width"],"selector":"input[mx-guid=\"xf962-\u001f\"]","attrs":[{"n":"style","v":"width:<%=width-10%>px"}]}],
    ctor: function(extra) {
        var me = this;
        Monitor.setup();
